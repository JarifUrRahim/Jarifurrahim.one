...
const manuscriptDir = path.join(root, 'edition-2', 'manuscript');
const outputDir = path.join(root, 'edition-2', 'chapters');
...
function renderMarkdown(markdown) {
  ...
  const flushTable = () => {
    ...
    if (parsed.length > 1) html.push(`<div class="table-wrap"><table><thead><tr>${parsed[0].map(cell => `<th>${inline(cell)}</th>`).join('')}</tr></thead><tbody>${parsed.slice(1).map(row => `<tr>${row.map(cell => `<td>${inline(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`);
    table = [];
  };
  ...
  for (const line of lines) {
    ...
    if (heading) { flush(); html.push(`<h${Math.min(heading[1].length + 1, 5)}>${inline(heading[2])}</h${Math.min(heading[1].length + 1, 5)}>`); continue; }
    if (/^>\s+/.test(line)) { flush(); html.push(`<blockquote><p>${inline(line.replace(/^>\s+/, ''))}</p></blockquote>`); continue; }
